...
class GoalDetailScreen extends ConsumerWidget {
  final int goalId; // 0 for goal_a, 1 for goal_b (legacy support)
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: goalsAsync.when(
          ...
          error: (e, _) => Center(child: Text('${AppLocalizations.get(locale, 'common_error')}: $e')),
          data: (goals) {
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Text('$progressPercent%', style: AppTypography.metric(context, color: accentColor)),
            ],
          ),
          const SizedBox(height: 24.0),
          Row(
          ...
          const SizedBox(height: 20.0),
          ProgressBar(progress: progressRatio, color: accentColor, height: 8.0),
        ],
      ),
    );
  }
  ...
  Widget _buildMetricColumn(BuildContext context, String label, String value) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(label, style: AppTypography.caption(context, color: AppColors.textSecondary(brightness))),
      ...
      data: (deposits) {
        return FutureBuilder<List<List<DepositAllocation>>>(
          ...
          builder: (context, snapshot) {
            ...
            final id = goal.id;
            final List<(Deposit, DepositAllocation)> filteredWithAlloc = [];
            for (int i = 0; i < deposits.length; i++) {
              final myAlloc = snapshot.data![i].where((a) => a.goalId == id).firstOrNull;
              if (myAlloc != null && myAlloc.amount > 0) {
                filteredWithAlloc.add((deposits[i], myAlloc));
              }
            }

            String forecastText = AppLocalizations.get(locale, 'goal_forecast_need_more');
            if (filteredWithAlloc.isNotEmpty) {
              int totalGoalDeposits = filteredWithAlloc.fold(0, (sum, pair) => sum + pair.$2.amount);
              final firstDate = filteredWithAlloc.last.$1.createdAt;
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(AppLocalizations.get(locale, 'goal_analytic_projection'), style: AppTypography.caption(context, color: AppColors.textSecondary(brightness))),
                        const SizedBox(height: 3.0),
                        Text(forecastText, style: AppTypography.bodySmall(context)),
                      ],
                    ),
                  ),
                ],
  ...
  Widget _buildTransactionHistory(BuildContext context, WidgetRef ref, String locale, String id, AsyncValue<List<Deposit>> depositsAsync, String currency, Color accentColor) {
    final brightness = Theme.of(context).brightness;
    return depositsAsync.when(
        ...
        return FutureBuilder<List<List<DepositAllocation>>>(
          ...
          builder: (context, snapshot) {
              ...
              return SurfaceCard(
                ...
                child: Center(
                  child: Text(
                    ...
                    style: AppTypography.bodySmall(context, color: AppColors.textTertiary(brightness)),
                  ),
                ),
              );
            }
            ...
            return ListView.builder(
              ...
              itemCount: filteredWithAlloc.length,
              itemBuilder: (context, index) {
                final dep = filteredWithAlloc[index].$1;
                final int amount = filteredWithAlloc[index].$2.amount;
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(dep.note ?? AppLocalizations.get(locale, 'goal_deposit_note'), style: AppTypography.bodySmall(context)),
                            const SizedBox(height: 3.0),
                            Text(_formatDate(dep.createdAt), style: AppTypography.caption(context, color: AppColors.textTertiary(brightness))),
                          ],
                        ),
                      ),
                      Text('+${formatAmount(amount)} $currency', style: AppTypography.amount(context, color: accentColor)),
                      if (isDeletable) ...[
                        const SizedBox(width: 8.0),
                        IconButton(
  ...
  Widget _buildPriceAnalysisLink(BuildContext context, String locale, Goal goal, Color accentColor) {
    return InkWell(
      borderRadius: BorderRadius.circular(16.0),
      onTap: () => context.push('/price-analysis', extra: {'query': goal.name, 'targetAmount': goal.targetAmount, 'currency': goal.currency}),
      child: SurfaceCard(
        padding: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 14.0),
        child: Row(
          children: [
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(AppLocalizations.get(locale, 'goal_price_scanner'), style: AppTypography.caption(context, color: AppColors.textSecondary(Theme.of(context).brightness))),
                  const SizedBox(height: 3.0),
                  Text('${AppLocalizations.get(locale, 'goal_price_check')}: «${goal.name}»', style: AppTypography.bodySmall(context)),
                ],
              ),
            ),
  ...
  Future<void> _confirmDelete(BuildContext context, WidgetRef ref, Deposit deposit) async {
    final brightness = Theme.of(context).brightness;
    final locale = ref.watch(localeProvider);
    showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
          ...
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16.0), side: BorderSide(color: AppColors.error, width: 1.5)),
          title: Text(AppLocalizations.get(locale, 'goal_reversal_title'), style: AppTypography.h3(context, color: AppColors.error)),
          content: Text(AppLocalizations.get(locale, 'goal_reversal_confirm'), style: AppTypography.bodySmall(context)),
          actions: [
            TextButton(
            ...
            TextButton(
              ...
              onPressed: () async {
                ...
                await ref.read(savingsNotifierProvider.notifier).deleteDeposit(deposit);
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(AppLocalizations.get(locale, 'goal_reversal_success')), backgroundColor: AppColors.error));
                }
              },
            ),
          ],
